...
O CT1 valida o carregamento inicial da tela de percurso, verificando a exibição dos componentes de telemetria, a apresentação dos valores iniciais zerados, o funcionamento da ação de iniciar o percurso, a atualização dos dados de telemetria durante a execução e a exibição dos gráficos de acompanhamento.

O CT2 verifica o fluxo de execução e reinicialização do percurso, garantindo que o sistema altere corretamente o estado da corrida após o início e que os dados sejam redefinidos após a confirmação da reinicialização.

O CT3 valida a finalização bem-sucedida do percurso, verificando a exibição da mensagem de sucesso ao alcançar o objetivo e a manutenção das métricas consolidadas apresentadas nos gráficos após o término da execução.
