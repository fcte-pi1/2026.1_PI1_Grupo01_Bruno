...
\begin{itemize}
    ...
    Para garantir a rastreabilidade e o cumprimento dos 35 requisitos funcionais definidos, os casos de uso foram detalhados e consolidados conforme a Tabela \ref{tab:casos_uso_resumo}.

    \begin{table}[htpb]
    \centering
    \caption{Especificação Detalhada e Rastreabilidade dos Casos de Uso.}
    \label{tab:casos_uso_resumo}
    \renewcommand{\arraystretch}{1.5} % Aumenta o espaçamento das linhas para a tabela ficar mais alta
    \footnotesize % Ajusta o tamanho da fonte para caber mais informação
    \begin{tabular}{|c|p{2.5cm}|p{2cm}|p{4.2cm}|p{4.2cm}|}
    \hline
    \textbf{ID} & \textbf{Caso de Uso} & \textbf{Ator(es)} & \textbf{Descrição do Fluxo Principal} & \textbf{Requisitos Vinculados (RFs)} \\ \hline
    
    \textbf{UC01} & Resolver Labirinto Autonomamente & Micromouse & O robô inicia a navegação, detecta paredes via ToF, calcula a rota, controla o PWM dos motores e aciona o buzzer ao chegar no centro. & [tof-paredes], [motor-pwm], [motor-direcao], [linha-reta], [curva], [botao], [buzzer], [disposição], [tamanho], [proteção], [modularidade], [chave-geral] \\ \hline
    
    \textbf{UC02} & Monitorar Telemetria Web & Usuário, Micromouse & O sistema web estabelece conexão via WebSocket e exibe o mapa dinâmico, trajeto, velocidade e o consumo atualizado de bateria. & [wifi-telemetria], [mapeamento\_trajeto\_web], [tipo\_labirinto\_web], [resto\_bateria\_web], [bateria\_consumida\_web], [tempo\_web], [status\_web], [vmedia\_web], [obstaculos\_web], [obstaculos\_id\_web], [map\_lab\_web], [websocket], [display-local] \\ \hline
    
    \textbf{UC03} & Gerenciar Operação Remota & Usuário & Através da web, o usuário aciona o robô, inicia o temporizador, reseta o sistema e envia logs de energia para persistência. & [on\_off\_web], [on\_temp\_web], [reset\_temp\_web], [dados\_bd\_web], [bat-log] \\ \hline
    
    \textbf{UC04} & Analisar Histórico e Energia & Usuário & O usuário consulta o banco de dados para avaliar trajetos passados, velocidade média e atuação dos mecanismos de proteção de bateria. & [salvar\_bd], [consulta\_bd], [bat-alerta], [power-throttling], [bat-proteção] \\ \hline
    \end{tabular}
    \end{table}

    \textbf{Priorização MoSCoW:} 
    \begin{itemize}
        \item \textbf{Must Have:} Navegação autônoma, mapeamento lógico, sensoriamento de paredes e transmissão de dados via WebSocket.
        \item \textbf{Should Have:} Persistência em banco de dados, dashboard de visualização web e controle remoto de operação.
        \item \textbf{Could Have:} Sinalização sonora ao atingir objetivo e transição para modo de economia de energia (Power Throttling).
    \end{itemize}
