...
void drawMaze()
{
    ...
    printf("XAROPi Mini - Flood Fill 4x4 | Passo: %d | Pos: (%d,%d) | Dir: %s | Dist: %d\n",
        ...
        for (int x = 0; x < SIZE; x++)
        {
            if (maze[x][y] & WALL_WEST)
            {
                if (knownMaze[x][y] & WALL_WEST)
                    printf(BRIGHT_RED "|" COLOR_RESET); // Conhecida
                else
                    printf(DARK_RED "|" COLOR_RESET); // Desconhecida
            }
            else
            {
                printf(" ");
...
void decidirEMover()
{
    ...
    uint8_t current_walls = knownMaze[current_x][current_y];
...
int main()
{
    ...
    floodFill();
    drawMaze();
    printf("\nLabirinto 4x4 carregado. Micromouse em (0,0) olhando NORTE.\n");
